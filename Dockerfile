FROM ubuntu
RUN apt-get update && apt-get -y install build-essential zip unzip tini

RUN apt-get update && \
  apt-get install -y --no-install-recommends \
  systemd systemd-sysv dbus dbus-user-session

## it is possible to to run with systemd if we include this AND use the --privileged flag at runtime: 
...
# Or just grab the deb
COPY xagt_36.30.0-1.ubuntu16_amd64.deb .
...
# docker run --rm -ti --init --name xg_agent xg_agent bash
